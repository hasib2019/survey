...
<main class="app-content">
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          <form action="@if (Auth::user()->is_admin==1){{ route('admin.village_update') }} @elseif(Auth::user()->is_admin==2) {{ route('village_update') }} @endif" method="post">
            @csrf
            <input type="hidden" name="id" id="" value="{{$village->id}}" readonly>
          <div class="row">
                ...
                <div class="form-row">
                  <div class="form-group col-md-6">
                    ...
                    <select name="district_id" id="demoSelect" class="form-control common" required>
                      ...
                      <option value="{{$districts->id}}" @if($districts->id==$village->district_id) selected @endif>{{$districts->district_name_bangla}}</option>
                      @endforeach
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <select name="upazilla_id" id="upazila" class="form-control" required>
                      <option value="">-----সিলেক্ট করুন------</option>
                      @foreach ($upazila as $upazilas)
                      <option value="{{$upazilas->id}}" @if($upazilas->id==$village->upazila_id) selected @endif>{{$upazilas->upazila_name_bangla}}</option>
                      @endforeach
                    </select>
                  </div>
                ...
                <div class="form-row">
                  <div class="form-group col-md-6">
                    ...
                    <select name="union_id" id="union" class="form-control" required>
                      ...
                      <option value="{{$unions->id}}" @if($unions->id==$village->union_id) selected @endif>{{$unions->union_name_bangla}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="form-group col-md-6">
                    <label>village</label>
                    <input type="text" name="name" id="" value="{{$village->village_name}}" class="form-control" placeholder="ইংরেজিতে লিখুন">
                  </div>
                </div>
                <div class="form-row">
                  <div class="form-group col-md-6">
                    <label>গ্রাম</label>
                    <input type="text" name="bn_name" id="" value="{{$village->village_name_bangla}}" class="form-control" placeholder="বাংলায় লিখুন">
                  </div>
                  <div class="form-group col-md-6">
                    <label>URL</label>
                    <input type="text" name="url" id="" value="{{$village->url}}" class="form-control" placeholder="Type url if exit">
                  </div>
                </div>
              </div>   
          </div>
